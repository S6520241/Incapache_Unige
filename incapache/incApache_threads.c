...
/*** TO BE DONE 8.1 START ***/
		...
		if (to_join[thrd_no] != NULL) {
			i = to_join[thrd_no] - thread_ids;  // ricava l'indice i del thread precedente
			conn_no = connection_no[thrd_no];  // usa la variabile già esistente
			pthread_mutex_unlock(&threads_mutex);

			pthread_join(thread_ids[i], NULL); // aspetta il thread precedente
...
/*** TO BE DONE 8.1 START ***/
	...
	for (int i = new_thread_idx - 1; i >= MAX_CONNECTIONS; i--) {
		if (connection_no[i] == connection_idx) {
			to_join[new_thread_idx] = &thread_ids[i];
			break;
		}
	}
/*** TO BE DONE 8.1 END ***/
