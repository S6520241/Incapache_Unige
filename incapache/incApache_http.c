...
#define HTML_404 "www-root/404_Not_Found.html"
#define HTML_501 "www-root/501_Method_Not_Implemented.html"
...
/*** TO BE DONE 8.0 START ***/
		method_str = strtok_r(http_request_line, " \t\r\n", &strtokr_save);
...
/*** TO BE DONE 8.0 START ***/
					...
					if (option_val != NULL && strncmp(option_val, "UserID=", 7) == 0) {
						UIDcookie = atoi(option_val + 7);
					}
/*** TO BE DONE 8.0 END ***/
                            }
...
/*** TO BE DONE 8.0 START ***/
					if (strcmp(option_name, "If-Modified-Since") == 0) {
						option_val = strtok_r(NULL, "\r\n", &strtokr_save);
						if (option_val != NULL && strptime(option_val, "%a, %d %b %Y %T GMT", &since_tm)) {
...
/*** TO BE OPTIONALLY CHANGED END ***/
		...
		if (access(filename, R_OK) != 0 || stat(filename, stat_p) < 0) {
			...
			if (MethodIsConditional(http_method)) {
				 ...
				 ***/
/*** TO BE DONE 8.0 START ***/
				time_t client_time = timegm(&since_tm);
				if (stat_p->st_mtime <= client_time) {
